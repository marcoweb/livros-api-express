...
class RequestHandler extends base_1.APIHandlerBase {
    ...
    buildUrlPatterns(idDivider, urlSegmentNameCharset) {
        const options = { segmentValueCharset: urlSegmentNameCharset };
        return {
            // collection operations
            collection: new url_pattern_1.default('/:type', options),
            // single resource operations
            single: new url_pattern_1.default('/:type/:id', options),
            // related entity fetching
            fetchRelationship: new url_pattern_1.default('/:type/:id/:relationship', options),
            // relationship operations
            relationship: new url_pattern_1.default('/:type/:id/relationships/:relationship', options),
        };
    }
    ...
    processRequestBody(type, requestBody, zodSchemas, mode) {
        ...
        if (attributes) {
            ...
            // if available
            const schemaName = `${(0, upper_case_first_1.upperCaseFirst)(type)}${(0, upper_case_first_1.upperCaseFirst)(mode)}ScalarSchema`;
    ...
    injectCompoundId(model, items) {
        const typeInfo = this.typeMap[(0, lower_case_first_1.lowerCaseFirst)(model)];
        if (!typeInfo) {
            return;
        }
